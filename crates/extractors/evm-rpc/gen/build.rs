fn main() -> Result<(), Box<dyn std::error::Error>> {
    #[cfg(gen_schema_manifest)]
    {
        ...
        let manifest_schema = schemars::schema_for!(evm_rpc_datasets::Manifest);
        ...
        println!(
            "cargo:warning=Generated EVM RPC dataset manifest schema file: {}",
    ...
    #[cfg(gen_schema_tables)]
    {
        ...
        let markdown = rt.block_on(async {
            datasets_raw::schema::to_markdown(evm_rpc_datasets::tables::all("test_network")).await
        });
        let out_dir = std::env::var("OUT_DIR")?;
